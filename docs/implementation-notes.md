...
Mechanical living-wiki evolve (entity/topic/source-summary/cross-source/comparison/event pages), refine-validate, rebuildable FTS search, dimensional perspective compare, source-gap explore, MCP, vault `lint`, offline eval corpus, exclusive write locking, proposal list/apply/reject, desk `init`, tar.gz `backup`/`restore` (data out of product Git), unattended `maintain once|loop` (Compose `maintainer` profile, durable `system/jobs/`), domain-neutral cross-MCP composition (`compose contract|join`, MCP `compose_contract` / `compose_with_external`), user-owned `memory/workspaces` (`workspace init|list|get|add-page|refine|benchtest`, protected from wiki evolve), and demand-driven `explore compile-from-ask` → `compile_from_ask_proposal` are implemented. LLM observation extraction and OCR/STT remain open.
