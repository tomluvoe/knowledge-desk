...
Mechanical wiki evolve/refine-validate, rebuildable FTS search, dimensional perspective compare, and source-gap explore (`gaps` / `ask` with optional update-queue proposals) are implemented. LLM extraction and full single-writer proposal application (#8) remain open. Semantic `lint` beyond wiki refine-validate remains deferred under #3.
